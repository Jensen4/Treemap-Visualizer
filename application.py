...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        if event_data["type"] == "call":
            src_n = event_data["src_number"]
            dst_n = event_data["dst_number"]
            t = datetime.datetime.strptime(event_data['time'],
                                           "%Y-%m-%d %H:%M:%S")
            if t.month != billing_month:
                new_month(customer_list, t.month, t.year)
                billing_month = t.month
            dur = event_data["duration"]
            src_l = tuple(event_data["src_loc"])
            dst_l = tuple(event_data["dst_loc"])
            temp = Call(src_n, dst_n, t, dur, src_l, dst_l)
            find_customer_by_number(src_n, customer_list).make_call(temp)
            find_customer_by_number(dst_n, customer_list).receive_call(temp)
