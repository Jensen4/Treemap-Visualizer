...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        filtered = []
        for call in data:
            if call.dst_number in cust or call.src_number in cust:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        specified in the handout.
        """
        LOWER_LONG = -79.697878
        UPPER_LONG = -79.196382
        LOWER_LAT = 43.576959
        UPPER_LAT = 43.799568
        ...
        for c in coord:
            try:
                ...
                return data
        coord = [float(n.strip()) for n in coord]
        if len(coord) > 4:
            return data

        if (LOWER_LONG <= coord[0] <= UPPER_LONG
                and LOWER_LONG <= coord[2] <= UPPER_LONG
                and LOWER_LAT <= coord[1] <= UPPER_LAT
                and LOWER_LAT <= coord[3] <= UPPER_LAT
                and coord[2] + coord[3] >= coord[0] + coord[1]):
            new = []
            for call in data:
                if (coord[0] <= call.src_loc[0] <= coord[2]
                        and coord[1] <= call.src_loc[1] <= coord[3]
                        or coord[0] <= call.dst_loc[0] <= coord[2]
                        and coord[1] <= call.dst_loc[1] <= coord[3]):
                    new.append(call)
            return new
        else:
            return data
